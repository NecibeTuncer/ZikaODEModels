function curve_fitting_ZIKA_Model25SINT_R1
...
%k =1.0e+03 *[2.551547085324936   0.093007119888124   0.000100000008075   0.000000000000033   0.385140893575393   0.000000000219522...
 %            0.000020000001438];
...
dy(3) = Lambda - beta_v* Iv*S/(S+I+R+Sp +Ip) -beta_d * S * (I+Ip)/(S+I+R+Sp +Ip)- (mu+xi)*S + omega*R +(1/280)*Sp;
dy(7) = xi*S - beta_vp * Iv * Sp/(S+I+R+Sp +Ip) - beta_dp * I * Sp/(S+I+R+Sp +Ip) - mu*Sp -(1/280)*Sp;
dy(4) = beta_v*Iv*S/(S+I+R+Sp +Ip) + beta_d * S *(I+Ip)/(S+I+R+Sp +Ip) - (mu + gamma)*I;
dy(8) = beta_vp * Iv * Sp/(S+I+R+Sp +Ip) +beta_dp * I * Sp/(S+I+R+Sp +Ip) -(mu+gammap) *Ip;
dy(5) = gamma*I +gammap * Ip - (mu + omega)*R;
dy(6) = beta_v* Iv*S/(S+I+R+Sp+Ip) +  beta_d * S * (I+Ip)/(S+I+R+Sp+Ip)+beta_vp * Iv * Sp/(S+I+R+Sp+Ip) +beta_dp * I * Sp/(S+I+R+Sp+Ip);
dy(9) = beta_vp * Iv * Sp/(S+I+R+Sp+Ip) +beta_dp * I * Sp/(S+I+R+Sp+Ip);
end
...
lb = [0        0       1/10  0   0       0      1/50 ];
ub = [1000000  100000  0.5  100  1000000 100000 1/7 ];
...
%The results
k =1.0e+03 *[2.617716723300200   0.091377567889342   0.000100000001346...
